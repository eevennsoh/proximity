...
func docCommand() *cli.Command {
	return &cli.Command{
		Name:   "doc",
		Usage:  "Show available Anthropic proxy endpoints",
		Action: doc,
	}
}
...
func writeDocumentation(output io.Writer, cfg *config.Config, port int) error {
	...
	if _, err := fmt.Fprintf(output, "Proximity Anthropic Proxy\n\nBase URL: %s\n\n", baseUrl); err != nil {
		return err
	}
	...
	if _, err := fmt.Fprintln(output, "  proximity anthropic login"); err != nil {
		return err
	}

	if _, err := fmt.Fprint(output, "\nClients do not need an Anthropic API key. Proximity injects Claude OAuth credentials.\n\n"); err != nil {
		return err
	}
...
// writeEndpointExamples writes curl examples for known Anthropic-compatible endpoints.
func writeEndpointExamples(output io.Writer, baseUrl string) error {
	...
	return writePostExample(output, baseUrl, "/v1/messages", `{"model":"claude-sonnet-4-5","max_tokens":256,"messages":[{"role":"user","content":"Say hello"}]}`)
}

// writeGetExample writes a single curl example for a GET endpoint.
func writeGetExample(output io.Writer, baseUrl string, path string) error {
...
func writePostExample(output io.Writer, baseUrl string, path string, body string) error {
